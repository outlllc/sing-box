...
# 上传文件
def upload_file(tb, chat_id, file_dir, caption = None):
  doc = open(file_dir, 'rb')
  if caption is None:
    tb.send_document(chat_id, doc)
  else:
    tb.send_document(chat_id, doc, caption = caption)
...
# 上传
def upload(tb, chat_id, dir, caption = None):
  if tb is None:
  ...
  if os.path.isfile(dir):
    upload_file(tb, chat_id, dir, caption = caption)
  else:
...
if __name__ == '__main__':
  ...
  if len(argv) == 3:
    ...
    upload(tb, chat_id, upload_path, caption = argv[3])
